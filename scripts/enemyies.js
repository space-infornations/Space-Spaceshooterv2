...
function drawAsteroids() {
  for (let i = 0; i < asteroids.length; i++) {
    ...
    ctx.beginPath();
    ctx.arc(
      asteroids[i].x + asteroidWidth / 2,
      asteroids[i].y + asteroidHeight / 2,
      asteroidWidth / 2,
      0,
      Math.PI * 2
    );
...
function detectBulletAsteroidCollisions() {
  const asteroidCollisionSound = new Audio("collision.mp3"); // Ensure the sound file is correct
  ...
  for (let i = bullets.length - 1; i >= 0; i--) { // Loop backwards for bullets
    for (let j = asteroids.length - 1; j >= 0; j--) { // Loop backwards for asteroids
      if (
        ...
        // Calculate damage based on bullet properties with a 5% increase
        const modifiedDamage = bullets[i].damage * 1.05; // Increase damage by 5%
        ...
        if (asteroids[j].health <= 0) {
          ...
          score += Math.ceil(5 * 1.05); // Adjust score for destroying an asteroid by 5%
        }

        asteroidCollisionSound.play(); // Play collision sound
...
function detectAsteroidPlayerCollisions() {
  for (let j = asteroids.length - 1; j >= 0; j--) { // Loop backwards for asteroids
    if (
      ...
      player.health -= totalDamage; // Reduce player's health
      ...
      // Optionally, you can remove the asteroid or apply other effects
