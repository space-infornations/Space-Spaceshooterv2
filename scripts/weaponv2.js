...
let baseCooldownPeriod = 500; // Base cooldown period for electric bullets in milliseconds
...
function shootElectricBullet() {
  if (canShootElectric()) {
    ...
    const centerY = player.y + player.height / 2; // Assuming the spaceship has a height
    ...
    if (battery >= totalBatteryConsumption) {
      for (let i = 0; i < numberOfBullets; i++) {
        ...
        electricBullets.push({
          x: centerX - electricBulletWidth / 2,
          y: centerY + i * bulletSpacing - (numberOfBullets - 1) * bulletSpacing / 2,
