...
function drawEnemies() {
  for (var i = 0; i < enemies.length; i++) {
    if (enemies[i].isFrozen) {
      ...
      var gradient = ctx.createLinearGradient(enemies[i].x, enemies[i].y, enemies[i].x, enemies[i].y + enemies[i].height); // Manually set the gradient stops based on the enemy's gradient
    ...
    ctx.fill(); // Reset the color for the next enemy
